...
def convert_to_sharegpt(path: str):
    total = get_file_line_count(path)
    if total == -1:
        raise ValueError('{} error'.format(path))
    f_name = os.path.basename(path)
    dir_ = os.path.dirname(path)
    save_f = open(os.path.join(dir_, 'sharegpt_{}'.format(f_name)), 'w')
    with open(path, 'r', encoding='utf-8') as f:
        for line in tqdm(f, total=total):
            ...
            new_conversation = []
            new_conversation.append({'content': human, 'role': 'human'})
            new_conversation.append({'content': assistant, 'role': 'assistant'})
